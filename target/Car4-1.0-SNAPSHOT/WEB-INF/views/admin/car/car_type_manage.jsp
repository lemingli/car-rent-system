    ...
    <div class="row">
        ...
        <div class="col-md-10">
            ...
            <table class="table table-striped table-bordered">
                ...
                <tr>
                    <th scope="col" style="min-width: 50px">车型名称</th>
                    <th scope="col" style="min-width: 50px">品牌</th>
                    <th scope="col" style="min-width: 50px">档位类型</th>
                    <th scope="col" style="min-width: 50px">天窗类型</th>
                    <th scope="col" style="min-width: 50px">座椅类型</th>
                    <th scope="col" style="min-width: 50px">倒车雷达</th>
                    <th scope="col" style="min-width: 50px">排量</th>
                    <th scope="col" style="min-width: 50px">油箱容量</th>
                    <th scope="col" style="min-width: 50px">音箱</th>
                    <th scope="col" style="min-width: 50px">价格</th>
                    <th scope="col" style="min-width: 50px">车量类型</th>
                    <th scope="col" style="min-width: 50px">车辆款式</th>
                    <%--<th scope="col">创建时间
                      <div class="btn-group">
                ...
                <c:forEach var="carType" items="${carType}">
                    <tr>
                        ...
                        <td scope="row">${carType.type_name}</td>
                        <td scope="row">${carType.brand}</td>

						<!--档位  -->
                        <c:if test="${carType.gear_box == 0}">
   							<td scope="row">自动</td>
						</c:if>
                        <c:if test="${carType.gear_box == 1}">
   							<td scope="row">手动</td>
						</c:if>
						
						<!--天窗  -->
						<c:if test="${carType.sunroof == 0}">
   							<td scope="row">单天窗</td>
						</c:if>
                        <c:if test="${carType.sunroof == 1}">
   							<td scope="row">全景天窗</td>
						</c:if>
                    	<c:if test="${carType.sunroof == 2}">
   							<td scope="row">无</td>
						</c:if>
                    	
                    	<!--座椅  -->
                    	<c:if test="${carType.seat == 0}">
   							<td scope="row">皮革座椅</td>
						</c:if>
                        <c:if test="${carType.seat == 1}">
   							<td scope="row">织物座椅</td>
						</c:if>
						
						<!--倒车雷达  -->
                        <c:if test="${carType.reverse_sensor == 0}">
   							<td scope="row">无</td>
						</c:if>
                        <c:if test="${carType.reverse_sensor == 1}">
   							<td scope="row">有</td>
						</c:if>
						
                        
                        <td scope="row">${carType.displacement}</td>
                        <td scope="row">${carType.fuel_tankage}</td>
                        <td scope="row">${carType.sound_box}</td>
                        <td scope="row">${carType.price}</td>


                        <!--车型  -->
						<c:if test="${carType.car_size == 0}">
   							<td scope="row">三厢</td>
						</c:if>
                        <c:if test="${carType.car_size == 1}">
   							<td scope="row">7座</td>
						</c:if>
                    	<c:if test="${carType.car_size == 2}">
   							<td scope="row">SUV</td>
						</c:if>
                       
	                    <!--档次  -->
						<c:if test="${carType.style == 0}">
   							<td scope="row">经济型</td>
						</c:if>
                        <c:if test="${carType.style == 1}">
   							<td scope="row">商务型</td>
						</c:if>
                    	<c:if test="${carType.style == 2}">
   							<td scope="row">豪华型</td>
						</c:if>
                        ...
                        <td>
                                ...
                                <input type="button" class="btn btn-info" value="修改" onclick="location.href='${ctx}/admin/car/${car.type_id}/modify'" />
